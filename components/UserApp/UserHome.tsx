...
import { Play, Radio, Newspaper, ChevronRight, Info } from 'lucide-react';
  ...
  const renderHome = () => (
    ...
    >
      ...
      <motion.section 
        ...
        className="relative h-64 md:h-[450px] rounded-2xl overflow-hidden group cursor-pointer border border-slate-800 shadow-2xl"
      ...
      {/* Category Scroll */}
      <motion.section variants={itemVariants} className="no-scrollbar overflow-x-auto flex gap-3 pb-2">
        <button 
      ...
      <motion.section variants={itemVariants}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => onTabChange('videos')} className="text-sm text-red-500 font-medium flex items-center gap-1">
          ...
          <AnimatePresence mode="popLayout">
            {filteredVideos.length > 0 ? filteredVideos.map((video) => (
              ...
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="col-span-full py-10 text-center text-slate-500 italic"
              >
      ...
      <motion.section variants={itemVariants}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => onTabChange('news')} className="text-sm text-blue-500 font-medium flex items-center gap-1">
            Mais notícias <ChevronRight size={14} />
          </button>
        </div>
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-4">
        {['Todos', ...Object.values(Category)].map(c => (
           <button 
             ...
             className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-all ${selectedCategory === c ? 'bg-red-600 border-red-600' : 'bg-slate-800 border-slate-700 hover:bg-slate-700'}`}
           >
             {c}
           </button>
        ))}
              ...
              <div className="divide-y divide-slate-800/50">
                {state.schedule.map((item) => (
                  <motion.div 
                    ...
                    key={item.id} 
                    className="grid grid-cols-12 p-5 items-center transition-colors group"
                  >
        ...
        <div className="space-y-12">
          <div className="min-h-[400px] flex flex-col items-center justify-center bg-slate-900/50 border-2 border-dashed border-slate-800 rounded-[3rem] p-12 text-center animate-in fade-in zoom-in-95 duration-700">
            <div className="w-24 h-24 bg-slate-950 rounded-full flex items-center justify-center mb-6 shadow-2xl border border-slate-800">
              <Radio size={48} className="grayscale opacity-30" />
            </div>
            ...
            <button 
              ...
              className="mt-8 bg-white text-slate-950 px-8 py-3 rounded-2xl font-black text-sm hover:bg-red-600 hover:text-white transition-all active:scale-95 shadow-xl"
            >
              VOLTAR AO INÍCIO
            </button>
          </div>
...
const VideoCard: React.FC<{ video: Video, onPlay: () => void }> = ({ video, onPlay }) => (
  ...
  >
    <div className="relative aspect-video rounded-xl overflow-hidden bg-slate-900 border border-slate-800 shadow-lg">
      ...
      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
        <motion.div 
          ...
          whileTap={{ scale: 0.9 }}
          className="w-12 h-12 bg-red-600 rounded-full flex items-center justify-center shadow-xl text-white"
        >
    ...
    <div>
      ...
      <p className="text-xs text-slate-500 mt-1">{video.views.toLocaleString()} visualizações • {video.category}</p>
    </div>
  </motion.div>
);
...
const NewsCard: React.FC<{ news: News, onClick: () => void }> = ({ news, onClick }) => (
  <motion.div 
    whileHover={{ y: -4, backgroundColor: 'rgba(30, 41, 59, 0.8)' }}
    onClick={onClick}
    className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden flex flex-col md:flex-row gap-4 p-4 transition-all cursor-pointer group shadow-lg"
  >
    <div className="w-full md:w-48 h-32 overflow-hidden rounded-xl flex-shrink-0">
